...
## function to generate stream data with imported Miyamoto-Nagai disk data
...
# Assuming the input is in the form "python train.py [1000,2000,3000] 1.0e-1"
arg = sys.argv[1]
...
for train_size in train_sizes:
    ...
    for i in range (0,n_flow):
        ...
        #arr_pos = plummer_and_stream(train_size, fraction_stream, seed=dataseeds[i])[:, 1:].astype(np.float32)
        arr_pos = arr_pos_prep[i*train_size:(i+1)*train_size]
        ...
        path_temp_wgt = "./wgt_maf_"+str(train_size)+'_'+str(fraction_stream)+"_data"#path_temp_wgt = path_temp_wgts[i]
        #path_temp_wgt = "./wgt_maf_"+str(train_size)+"_data"#path_temp_wgt = path_temp_wgts[i]
        path_log_file = "./wgt_maf_"+str(train_size)+"_"+str(fraction_stream)+"_data.log"#path_log_file = path_log_files[i]
        ...
        with open(path_log_file, 'w') as fp:
            ...
            np.save(str(train_size)+'_'+str(fraction_stream)+'_datavariance_losses.npy',losses) # updates and saves best loss at the end of every flow training loop
